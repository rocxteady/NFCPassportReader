...
public class TagReader {
    ...
    func send( cmd: NFCISO7816APDU, useExtendedMode : Bool = false ) async throws -> ResponseAPDU {
        ...
        Log.debug( "TagReader - Received response, size \(data.count)b" )
        ...
        while (sw1 == 0x61) {
            ...
            Log.debug("Read remaining data. Accumulated: \(data.count + nextSegment.count)b. Last batch \(nextSegment.count)b. Still remaining: \(sw2)b")
            data += nextSegment
        }
